package com.Sunrise.Services.StorageDataServices.Interfaces;

import com.Sunrise.Entities.User;
import com.Sunrise.Services.StorageDataServices.CacheService;

import java.util.List;
import java.util.Optional;
import java.util.Set;

public interface ICacheStorageService extends IStorageService {


    // ========== ACTIVE USER METHODS ==========

    Optional<User> getActiveUser(String jwt);
    void saveActiveUser(String jwt, Long userId);
    void deleteActiveUser(String jwt);


    // ========== CHAT MEMBER METHODS ==========

    Optional<Boolean> isGroupChat(Long chatId);
    Optional<List<Long>> getUserChats(Long userId);
    Set<Long> getChatMembers(Long chatId);
    void saveUser(User user);
    void enableUser(Long userId);
    void addUserToChat(Long userId, Long chatId);


    // ========== ADMIN RIGHTS METHODS ==========

    Set<Long> getChatAdmins(Long chatId);


    // ========== CACHE MANAGEMENT ==========

    CacheService.CacheStats getStats();
    void clearUserCache();
    void clearChatCache();
    void clearVerificationTokenCache();
    void clearAll();
}
